...
//        $this->db->where_in('ListID', $list_id);
    ...
    public function get_bbFilterCustomer() {
        ...
        $query1 = "select * from bb_customer where MONTH(created) = (MONTH(CURDATE())-1) ";
        $query2 = "select * from bb_customer where MONTH(created) = (MONTH(CURDATE())-2) ";
        $query3 = "select * from bb_customer where created >= DATE_SUB( CURDATE( ) , INTERVAL 30 DAY) ";
        $query4 = "select * from bb_customer where `created` between '" . date("Y-m-d", strtotime("-60 days")) . "' and '" . date("Y-m-d", strtotime("-30 days")) . "'";
        $query5 = "select * from bb_customer where `created` >= DATE_SUB( CURDATE( ) , INTERVAL 7 DAY) ";
        $query6 = "select * from bb_customer where `created` between DATE_SUB( CURDATE( ) , INTERVAL 14 DAY) and DATE_SUB( CURDATE( ) , INTERVAL 7 DAY) ";
        $query7 = "select * from bb_customer where DATE(`created`) = DATE(CURDATE( )) ";
        $query8 = "select * from bb_customer where DATE(`created`) = DATE(CURDATE( ) - 1) ";
//   echo $query8;die;
...
//        echo date("Y-m", strtotime("-0 months"));die;
        $data = array();
        $query9 = "select * from et_subscriber_list_rel 
        ...
        $query1 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID   
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID 
                   where MONTH( CURDATE( ) - 1) = MONTH(et_subscriber_list_rel.CreatedDate ) and et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        $query2 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID   
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID  
                   where  MONTH(et_subscriber_list_rel.`CreatedDate`) = (MONTH(CURDATE())- 2) and et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        $query3 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID 
                   where et_subscriber_list_rel.`CreatedDate` >= DATE_SUB( CURDATE( ) , INTERVAL 30 DAY ) and et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        ...
        $query5 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID
                   where DATE(et_subscriber_list_rel.`CreatedDate`) = DATE(CURDATE()) and  et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        $query6 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID
                   where DATE(et_subscriber_list_rel.`CreatedDate`) = DATE(CURDATE() - 1) and  et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        $query7 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID
                   where et_subscriber_list_rel.`CreatedDate` >= DATE_SUB( CURDATE( ) , INTERVAL 7 DAY ) and  et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
        $query8 = "select * from et_subscriber_list_rel 
                   JOIN master_subscriber ON master_subscriber.ET_UID = et_subscriber_list_rel.SubscriberID
                   JOIN bb_customer ON bb_customer.BB_UID = master_subscriber.BB_UID
                   where et_subscriber_list_rel.`CreatedDate` between (DATE_SUB( CURDATE( ) , INTERVAL 14 DAY ) and DATE_SUB( CURDATE( ) , INTERVAL 7 DAY )) and  et_subscriber_list_rel.`ListID`  = '" . $list_id . "'";
